...
def get_azure_connection(context):
    ...
    blob_details = {
        ...
        "host":"https://dlsdatalinkdppilot.blob.core.windows.net",
        "connection_string":"BlobEndpoint=https://dlsdatalinkdppilot.blob.core.windows.net/;QueueEndpoint=https://dlsdatalinkdppilot.queue.core.windows.net/;FileEndpoint=https://dlsdatalinkdppilot.file.core.windows.net/;TableEndpoint=https://dlsdatalinkdppilot.table.core.windows.net/;SharedAccessSignature=sv=2021-12-02&ss=bfqt&srt=sco&sp=rwdlacupyx&se=2024-04-18T02:22:20Z&st=2023-04-17T18:22:20Z&spr=https,http&sig=rKdcxg6Xn1YMW6CCBkCfZuXEf7JLsdRK6g%2BLRDuXFhA%3D"
        }
...
def format_columns(columns):
    ...
    for col in columns:
        ...
        if isinstance(col, (list, tuple)):
            col_list.append((
                col[0],
                re.sub(special_char_set,"_", col[1]).upper()
                ))
        else:
...
def process(**context):
    try:
        ...
        ls=[(file_dict, blob_i, file_columns, created, table_columns) for
...
with DAG(
    ...
    Process_Files_ADLS_Snowflake_XML = PythonOperator(
        task_id='Process_Files_ADLS_Snowflake_XML',
    ...
    with TaskGroup(group_id='Process_Files', dag=dag) as Process_Files:
        ...
        handle_blob_list_operator = PythonOperator.partial(
            ...
            dag=dag
        ).expand(op_args=Process_Files_ADLS_Snowflake_XML.output)


Process_Files_ADLS_Snowflake_XML >> handle_blob_list_operator
